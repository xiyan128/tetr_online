...
## v4 BATTERY VERDICT + A-r11 (2026-07-09): per-child rounds from the v4 lineage; the slot vehicle waits for on-policy data

Battery (16-game duels): (a) slot hit@12 **0.909** (bar met; 0.816→0.877→0.909 across epochs; sCE 4.24→2.79) ✓; (b) guided:v4 vs guided:v3 **5-11** ✗ (borderline; pre-reg line was ≥6/16); (c) CC2 anchor guided:v4 **11-5** ✓ (v3's control: 9-7 — v4 is anchor-stronger); (d) sguided:v4 vs CC2 **1-15 in 45s** ✗✗; (e) sguided-vs-guided 2-14 ✗. **m-dial refuted**: m16/m24 both 2-14 — headroom doesn't fix play.

Slot-collapse forensics (all exonerating code, indicting distribution): Rust `forward_slots` ≡ Python on real obs (NEW golden coverage: slots in golden_v2.json + `env_dir_fixture` probe; the head previously shipped unGoldened); pathless placements carry full pose+hold (lever #1 innocent); opp planes all-zero in corpus AND play (blind-by-rule); forced-death rate ~0 at m12 on corpus states. Same-seed trace: the slot filter drops the clean placement early (ply 2), mediocre choices compound into jagged boards, rain finishes it. **Corpus-marginal hit@k is the wrong sufficiency metric — the filter fails on the play distribution its own filtering creates.** The healing path is the campaign itself: each round's net-play parent states enter the mix, training the slot head on-policy; re-qualify `sguided` per round (the collapsed read costs 45s).

**A-r11 (logged before launch):** lineage = round0_v4 — FORCED, not preferential: fine-tunes inherit the checkpoint's whitening, so any v3 lineage would propagate the dead-parent-trunk defect indefinitely. Incumbent stays round0_v3 per the gate letter (v4 lost (b)). Vehicle = guided (per-child) end-to-end; games 600 (halved for cadence at per-child speed ~330/hr; revisit at sguided re-qualification). Everything else per the pre-registered tree.

## Round-11 in flight (2026-07-09): datagen receipts

First consistent-vehicle round (guided per-child, v4 lineage, v3 incumbent, 600 games). Datagen receipts: grounded half **net-seat 161-139 over driver-CC2** (53.7% — the v4 lineage is competitive in its own data plant; the new `net_opp_draw` field's first read); mirror half 159-141 seat-split (stagger fair), **111 shards vs the grounded half's 72 — mirror net-vs-net games run LONG** (healthy competitive play; the r1-era 63-ply suicide blitzes are gone with the honest vehicle). Cost: grounded 236 games/hr, mirror 92 games/hr at 6 workers — the serial halves cost ~4.5h wall, so round.py now runs the halves CONCURRENTLY on split workers (banked for round-12). Training on the 324-shard mix in progress; duels + gate next. Post-round protocol: ledger verdict per the pre-registered tree, then `requalify_slots.sh r11/net r11/corpus` (on-policy slot healing thesis — hit@k on the round's own shards + the 45s sguided anchor).
