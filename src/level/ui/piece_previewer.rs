...
use crate::level::{FallingBlock, LevelCleanup, LevelConfig, PieceHolder, spawn_free_block, to_translation};
...
pub fn spawn_piece_previewer(
    ...
    let piece_previewer_entity = commands
        ...
        .insert(SpatialBundle {
            ...
            Transform::from_translation(
                to_translation(config.board_width as isize, (config.board_height) as isize, config.block_size)
                    + offset
            ),
            ..Default::default()
        })
    ...
    for i in 0..3 {
        let box_entity = commands.spawn((
            SpatialBundle::default(), PreviewHolder { index: i })
        ).id();

        commands.entity(piece_previewer_entity).add_child(box_entity);
    }
}

pub fn spawn_hold_viewer(
    mut commands: Commands,
    config: Res<LevelConfig>,
) {
    let offset = Vec3::new(-calc_ui_offset(&config), 0., 0.);
    ...
    let hold_viewer = commands
        ...
        .insert(SpatialBundle {
            transform:
            Transform::from_translation(
                to_translation(0, (config.board_height) as isize, config.block_size)
                    + offset
            ),
            ..Default::default()
        })
    ...
    let box_entity = commands.spawn((
        SpatialBundle::default(), PreviewHolder { index: 0 })
    ).id();
...
pub fn update_piece_previewer(
    ...
    if children_query.is_empty() || !generator.is_changed() {
        return;
    }
    ...
    for child in children.iter() {
        ...
        let (preview_board_size, piece_entity) = spawn_holder_piece(&config, &game_assets, &mut commands, piece_type);
...
pub fn update_hold_viewer(
    ...
    for child in children.iter() {
        ...
        if let Some(piece_type) = piece_holder.piece.clone() {
            ...
            let (preview_board_size, piece_entity) = spawn_holder_piece(&config, &game_assets, &mut commands, piece_type.piece_type());
...
fn spawn_holder_piece(config: &Res<LevelConfig>, game_assets: &Res<GameAssets>, mut commands: &mut Commands, piece_type: PieceType) -> (Vec2, Entity) {
    let piece = Piece::from(piece_type);

    let avatar_board = piece.avatar_board();

    let preview_block_size = config.block_size * 0.8;

    let block_ids: Vec<Entity> = avatar_board.cells().iter().map(|&cell| {
        spawn_free_block(&mut commands, &LevelConfig {
            block_size: preview_block_size,
            ..Default::default()
        }, &game_assets, &piece, cell, FallingBlock)
    }).collect();
    ...
    let preview_board_size = Vec2::new(avatar_board.width() as f32, avatar_board.height() as f32)
        * preview_block_size * scale;


    let piece_entity = commands.spawn(piece).insert(
        SpriteBundle {
            sprite: Sprite {
