...
//!
//! This crate is the **ADR-7 boundary made into a crate boundary**: a pure,
//! deterministic Tetris rule [`engine`], a [`player`] controller abstraction, and
//! an [`ai`] bot — all with **no Bevy types**. What used to be enforced by
//! convention ("the engine carries no rendering or Bevy types") is now enforced by
//! the compiler: this crate does not depend on Bevy (except an optional, off-by-
