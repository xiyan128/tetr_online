...
- **[High] ENG-3.** `clear_full_rows` shift overflows if the clear loop reaches `y = 63`. `bit_board.rs` does `*col >> (y+1)` with `y` up to 63, a shift by 64 (UB or panic). Since the ENG-2 fix, the loop bound is `total_rows` (clamped to `.min(64)`), so the trigger is now `total_rows == 64` rather than the old `visible_rows >= 64`. Latent (production is 40, so `y` tops out at 39), but it is a `pub` constructor footgun on the hot path. Fix: `debug_assert!(total_rows <= 64)` plus `checked_shr` (or cap the backing height at 63).
