...
- `cleared_rows` over-reports buffer rows in both representations (see ENG-2).
...
Fix: add `from_board` composed with `to_array2d` round-trip property tests. Add tests that construct the buffer-row case and assert the scored line count, not just `bitboard == engine`. Enforce the 16x64 envelope at `Board` or `EngineConfig` (or `debug_assert` in `from_board`). Centralize occupancy on the existing `Occupancy` trait.
...
- **[High] ENG-2.** `cleared_rows` over-reports buffer-zone rows, and the value is consumed as the line count. `lock_clear.rs:77-83` (a full 64-bit scan) vs visible-bounded `clear_lines`. Consumers do `cleared_rows.len()` (`eval/mod.rs:234`, `state.rs:297`, `api.rs:509`). A full buffer row inflates score, combo, B2B, and reward while staying on the board. This is a sensitive area (the prior buffer-zone fix), so decide the contract explicitly rather than blind-fix: either clear and count buffer rows, or do neither, then pin it with a constructed test (the differential tests cannot catch it). It is reachable but rare, since it needs a piece to complete a hole in the vanish zone.
- **[High] ENG-3.** `clear_full_rows` shift overflows if `visible_rows >= 64`. `bit_board.rs:193` does `*col >> (y+1)` with `y` up to 63, a shift by 64 (UB or panic). `empty()` clamps `total_rows.min(64)` but not `visible_rows` (`:78`). Latent (production is 20), but it is a `pub` constructor footgun on the hot path. Fix: clamp `visible_rows`, `debug_assert!(visible_rows <= total_rows <= 64)`, or use `checked_shr`.
